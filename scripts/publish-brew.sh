...
FORMULA_SRC="${FORMULA_SRC:-$ROOT_DIR/homebrew/git-ai-commit.rb}"
...
cp "$FORMULA_SRC" "$TAP_DIR/Formula/git-ai-commit.rb"

cd "$TAP_DIR"
git add Formula/git-ai-commit.rb
...
version="$(grep -E '^\s*version "' Formula/git-ai-commit.rb | head -n1 | sed -E 's/.*version "([^"]+)".*/\1/')"
if [ -z "$version" ]; then
  version="unknown"
fi

git commit -m "chore: update git-ai-commit ${version}"
